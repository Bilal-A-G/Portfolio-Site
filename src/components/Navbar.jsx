...
const Navbar = () => {
    return(
      <nav className="bg-off-white flex w-full h-[56px] z-20">
          <div className="w-1/4 h-full">
              <div className="justify-center pb-2 h-full flex text-[18px] text-black font-bold tracking-wider px-2">
                  <NavbarLink select={false} extras={<><p className="font-[900]">()[]</p><p>Bilal</p><p>A-G</p></>} link={"/"} props={"space-x-2 px-2 pt-3 h-full"}/>
              </div>
          </div>
          <div className="w-3/4 ml-[220px] flex text-[16px] font-medium tracking-wider">
                  ...
                  name !== "Projects" ? <NavbarLink text={name} select={true} link={path} props={"px-3 pt-4 h-full"}/> : 
                      <NavbarLink text={name} select={true} props={"px-3 pt-4 h-full"} group={"group/top"}
                                  extras={<>
                                      ...
                                      <div className={"bg-off-white w-[180px] mt-[40px] absolute invisible group-hover/top:visible hover:!visible z-20 rounded-b-md"}>
